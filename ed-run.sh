...
tar -xvf /app/ed-panel.tar -C ~/.config/xfce4
...
cat << EOF >  /Desktop/ed-Swicth_to_Chinese_input.sh
#!/bin/bash
im-config -s ibus
ibus-setup
EOF
...
chmod +x /Desktop/ed-Swicth_to_Chinese_input.sh
exec supervisord -c /app/ed-supervisord.conf
